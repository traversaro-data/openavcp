...
def xml_filename(year,month,closed=True):
    if( closed ):
        ...
        return "avcp_contracts_"+str(year)+"_"+'%02d' % month+"_open.xml";
...
def extract_data_from_file(db,filename,pubblication_year,pubblication_month):
    ...
    for lotto in root.iter('lotto'):
        ...
        contract_row['pubblication_date'] = str(pubblication_year)+"-"+str(pubblication_month)
        ...
        if( lotto.find('aggiudicatari').text is not None ):
            if( lotto.find('aggiudicatari').find('aggiudicatario') is not None and lotto.find('aggiudicatari').find('aggiudicatario').text is not None ):
